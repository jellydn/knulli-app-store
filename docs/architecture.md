...
An archive is fully downloaded, hashed, and staged before a destination file changes. Every destination mutation enters the transaction and a durable journal before it happens. Installed state is the last file an operation writes; a health check may rewrite only the content signature it just verified, under the manager lock and only while the state it read is unchanged. A failed step restores snapshots in reverse order. The next locked start rolls back an open journal and discards a committed leftover directory.
...
Installed state records the mode observed after the destination copy, plus the size and modification time of the bytes each hash was verified against. Health checks compare immutable managed files with the reviewed content hash and require the recorded mode to keep its owner read, write, and execute permissions; a wider mode applied by the destination filesystem is accepted, because that filesystem owns the mode bits. A file that still matches its recorded signature is not read again, so a catalogue load hashes only the files whose metadata moved, and a check that verifies content records the signature it confirmed, which lets pre-signature state heal on its first load. Runtime files outside the release inventory do not become manager-owned and do not make a package unhealthy. A failed check has a structured path, check type, expected value, and actual value for the GUI and redacted log.
