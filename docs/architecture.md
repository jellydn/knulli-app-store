...
Installed state records the mode observed after the destination copy. Health checks compare immutable managed files with the reviewed content hash and require the recorded mode to keep its read, write, and execute capabilities; a wider mode applied by the destination filesystem is accepted, because that filesystem owns the mode bits. Runtime files outside the release inventory do not become manager-owned and do not make a package unhealthy. A failed check has a structured path, check type, expected value, and actual value for the GUI and redacted log.
