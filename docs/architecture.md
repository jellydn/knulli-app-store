...
Installed state records the mode observed after the destination copy, plus the size and modification time of the bytes each hash was verified against. Health checks compare immutable managed files with the reviewed content hash and require the recorded mode to keep its read, write, and execute capabilities; a wider mode applied by the destination filesystem is accepted, because that filesystem owns the mode bits. A file that still matches its recorded signature is not read again, so a catalogue load hashes only the files whose metadata moved. Runtime files outside the release inventory do not become manager-owned and do not make a package unhealthy. A failed check has a structured path, check type, expected value, and actual value for the GUI and redacted log.
