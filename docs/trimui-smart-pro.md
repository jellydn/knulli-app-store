...
Health checks hash every immutable managed release file. PlayTime's database, generated hook, and other runtime data do not make the install unhealthy. The installed state records each destination file's observed mode because a target filesystem can normalize the requested mode. A wider mode is normal there: the check fails only when a file loses the read, write, or execute permission it had at install time, so `Grout.sh` stays healthy when the card reports `0777` instead of the requested `0755`. Select **Issue** to open the health section directly. It shows the exact path, full expected and actual hash or mode, and Repair guidance.

If an operation fails, the Store records its safe retry target under `/userdata/system/knulli-app-store/lifecycle/`. A failed fresh PlayTime install with no external files shows **Retry install**, never **Retry manage**, and a destination that only holds empty directories counts as having no external files. Failed update, Repair, Manage existing, Force reinstall, and Uninstall operations return to their own action after restart only when current state and compatibility still allow it. Each retry repeats all mandatory checks.
...
The first launch for each device/controller identity opens a dedicated setup screen before the catalogue. Choose **Use Detected Mapping**, **Test Detected Mapping**, **Customize**, or **Safe Exit**. Every choice that builds a mapping asks one question first: whether this pad pages with the shoulder buttons. Paging is optional, so a pad without them answers **Skip paging** and setup asks only for the required actions — Navigate for up, down, left, and right, then Select, Back, and Settings — reviewing each detected button with Select, Retry, Start Over, and Back choices. A mapping that keeps paging asks for Page Up and Page Down afterwards, in the same walk. The preview is finished by the required actions, and an optional binding the user never pressed is left out of what is saved, so a mapping never claims a button the pad has not proved. Later launches load the saved mapping directly. Settings can reopen setup, export diagnostics, or reset the active mapping. Hold the saved Back and Settings controls while launching for a deliberate setup override. Physical button positions are not assumed.
...
The footer is the only place the interface shows controls. It names each action available on the current screen beside its binding, for example `Confirm (SOUTH)  Settings (NORTH)  Quit (SELECT + NORTH)` on the catalogue, and the same action name always means the same action: Confirm accepts or opens, Back cancels or steps back a panel, Settings opens controller settings. Face buttons use compass directions (north, south, east, west) rather than A/B/X/Y, because letter labels differ across device layouts. No screen repeats a control hint in its body text, and the controller mapping summary lists the same names beside the buttons that carry them. Quitting is the one control no button carries alone: hold Select and press North. On the blocked screen, Confirm exports diagnostics and the same chord is the way out.
