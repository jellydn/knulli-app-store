...
Health checks compare only immutable managed release files. PlayTime's database, generated hook, and other runtime data do not make the install unhealthy. The installed state records each destination file's observed mode because a target filesystem can normalize the requested mode. A wider mode is normal there: the check fails only when a file loses the read, write, or execute capability it had at install time, so `Grout.sh` stays healthy when the card reports `0777` instead of the requested `0755`. Content is hashed when a file's size or modification time no longer matches the signature recorded with its hash, so a catalogue load does not re-read files an operation already verified; the log reports how many files each check read. Select **Issue** to open the health section directly. It shows the exact path, full expected and actual hash or mode, and Repair guidance.
