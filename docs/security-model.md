...
- Content verification trusts the size and modification time recorded with the last verified hash. A change that preserves both, such as a rewrite that restores the timestamp, is not detected until the destination metadata moves. An adopted file that differs from the reviewed release records no signature and is hashed on every check.
...
- Health checks use the observed destination mode after transactional copy or adoption mode normalization, and require that mode to keep the owner read, write, and execute permissions the installer set. A wider mode from a normalizing filesystem is not a health failure; a lost owner permission is. They compare the full SHA-256 of every manager-owned immutable file whose size or modification time differs from the signature recorded with that hash, so a catalogue load does not re-read files that an operation already verified. A check that verifies content records the signature it confirmed under the manager lock, and only while the installed state it read is still on disk, so a load can heal pre-signature state without racing an operation. Unmanaged external files show their full reviewed and actual hashes for Repair instead of being treated as reviewed bytes.
