...
- Health checks use the observed destination mode after transactional copy or adoption mode normalization, and require that mode to keep the owner read, write, and execute permissions the installer set. A wider mode from a normalizing filesystem is not a health failure; a lost capability is. They compare the full SHA-256 of every manager-owned immutable file whose size or modification time differs from the signature recorded with that hash, so a catalogue load does not re-read files that an operation already verified. Unmanaged external files show their full reviewed and actual hashes for Repair instead of being treated as reviewed bytes.
