...
The first launch for each device/controller identity opens a dedicated setup screen before the catalogue. Choose **Use Detected Mapping**, **Test Detected Mapping**, **Customize**, or **Safe Exit**. Customize assigns the seven actions one at a time: Navigate for up, down, left, and right, then Select, Back, and Settings; quitting is the Select chord rather than a bound action. Every detected button gets a Select, Retry, Start Over, or Back review before progress continues. Conflicts stay on the current action. Custom and tested mappings require a complete preview before atomic save.
