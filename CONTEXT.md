...
**Health** — The check that managed files still match what was installed, in content and in the mode the filesystem actually applied. A wider mode applied by the destination filesystem is accepted; only a lost read, write, or execute capability fails. Preserved and unmanaged files are exempt.
