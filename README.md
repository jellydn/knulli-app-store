...
Package health checks report the exact managed path and full expected and actual hash or mode. Selecting **Issue** opens this health section before the action list and gives the Repair step. State records the mode that the destination filesystem applies, not only the requested staging mode, and the check tolerates a wider mode from a filesystem that normalizes permissions. State also records the size and modification time of each verified file, so a load hashes only the files whose size or time moved instead of re-reading every managed file; the log reports how many files each check read. Only a lost read, write, or execute capability is an issue, so a file the installer requested as `0755` and the SD card reports as `0777` stays healthy. This prevents a false Repair state when a Knulli filesystem normalizes permissions. PlayTime's runtime database, hook, and other data outside its three immutable release files do not cause a health failure.
